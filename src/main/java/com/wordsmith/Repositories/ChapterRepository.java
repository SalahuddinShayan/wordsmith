...
public interface ChapterRepository extends JpaRepository<Chapter, Long> {
	...
	@Query(value = "select min(chapter_id) from chapter where novel_name = :novelname and chapter_id > :chapterid AND release_status = 'RELEASED';", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM chapter WHERE release_status = 'RELEASED' order by chapter_id desc limit 20;", nativeQuery = true)
