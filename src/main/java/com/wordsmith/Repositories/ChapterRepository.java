...
public interface ChapterRepository extends JpaRepository<Chapter, Long> {
	...
	@Query(value = "SELECT * FROM chapter WHERE novel_name = :novelname order by chapter_no desc limit 3;", nativeQuery = true)
    List<Chapter>  Latest(@Param("novelname") String novelname);
	
	@Query(value = "SELECT * FROM chapter WHERE novel_name = :novelname order by chapter_no desc limit 1;", nativeQuery = true)
